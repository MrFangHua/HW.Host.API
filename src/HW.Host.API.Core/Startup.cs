...
using HW.Host.API.Infrastructure.Email;
using HW.Host.API.Infrastructure.LogDB;
using HW.Host.API.Infrastructure.Middleware.ExceptionHandl;
...
namespace HW.Host.API.Core
{
    public class Startup
    {
        ...
        public void ConfigureServices(IServiceCollection services)
        {
            ...
            services.AddDbContext<HWHostDbContext>();

            // 注册FangHuaHostLogDbContext为服务
            services.AddDbContext<HWHostLogDbContext>();
            ...
            {
                // 设置Swagger文档相关信息
                s.SwaggerDoc($"HW.Host.App.API", new Info
                {
                    Version = Version,
                    Title = $"{Project_Name} - HW.Host.App.API 默认接口文档",
                    Description = $"{Project_Name_English} - HW.Host.App HTTP/HTTPS API _{Version}",
                    // 服务条款
                    // TermsOfService = "None",
                    License = new License
                    {
                ...
                s.SwaggerDoc($"HW.Host.Admin.API", new Info
                {
                    Version = Version,
                    Title = $"{Project_Name} - HW.Host.Admin.API 默认接口文档",
                    Description = $"{Project_Name_English} - HW.Host.Admin HTTP/HTTPS API_{Version}",
                    // 服务条款
                    // TermsOfService = "None",
                    License = new License
                    {
                ...
                // 添加接口XML的路径
                var xmlPath = Path.Combine(basePath, "ExegesisXmlFile/HW.Host.API.Application.xml");
                // 定义实体类XML的路径
                var entityXmlPath = Path.Combine(basePath, "ExegesisXmlFile/HW.Host.API.Model.xml");
                ...
                var security = new Dictionary<string, IEnumerable<string>> { { "HW.Host.API", new string[] { } } };
                s.AddSecurityRequirement(security);

                s.AddSecurityDefinition("HW.Host.API", new ApiKeyScheme
                {
        ...
        public void Configure(IApplicationBuilder app, IHostingEnvironment env, HWHostDbContext context, ILoggerFactory logger)
        {
            ...
            app.UseSwaggerUI(s =>
            {
                s.SwaggerEndpoint($"/swagger/HW.Host.App.API/swagger.json", $"{Project_Name_English} - HW.Host.App HTTP/HTTPS API");
                s.SwaggerEndpoint($"/swagger/HW.Host.Admin.API/swagger.json", $"{Project_Name_English} - HW.Host.Admin HTTP/HTTPS API");
